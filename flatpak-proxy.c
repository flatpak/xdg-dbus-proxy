 ...
 *
 * Serial numbers received in messages from the client are remapped
 * to a monotonically increasing value tracked by the proxy before
 * they are sent to the bus. These differing serials shouldn't effect
 * the client; if the bus is expectedly replying to a method call 
 * from the client, the reply serial is remapped back to the expected
 * serial before it is sent back to the client.
 *
  ...
  while (g_hash_table_iter_next (&iter, &key, &value))
    {
      ...
      guint bus_serial_add_match = queue_fake_message (client, message, EXPECTED_REPLY_FILTER);

      if (client->proxy->log_messages)
        g_print ("C%d: -> org.freedesktop.DBus fake %sAddMatch for %s\n", bus_serial_add_match, name_needs_subtree ? "wildcarded " : "", name);
      ...
      if (!name_needs_subtree)
        {
          ...
          guint bus_serial_get_owner = queue_fake_message (client, message, EXPECTED_REPLY_FAKE_GET_NAME_OWNER);
          g_hash_table_replace (client->get_owner_reply, GINT_TO_POINTER (bus_serial_get_owner), g_strdup (name));

          if (client->proxy->log_messages)
            g_print ("C%d: -> org.freedesktop.DBus fake GetNameOwner for %s\n", bus_serial_get_owner, name);
        }
      else
